...
// AddFrom adds values from a map
func (h *Headers) AddFrom(m map[string][]string) {
	for key, values := range m {
		h.Add(key, values...)
	}
}
...
func (h *Headers) ValuesIter(key string) iter.Iterator[string] {
	valuesPairs := iter.Filter[Pair](h.Iter(), func(pair Pair) bool {
		return strcomp.EqualFold(pair.Key, key)
	})

	return iter.Map[Pair, string](valuesPairs, func(pair Pair) string {
		return pair.Value
	})
}
...
func (h *Headers) KeysIter() iter.Iterator[string] {
	keys := iter.Map[Pair, string](h.Iter(), func(pair Pair) string {
		return pair.Key
	})
...
func (h *Headers) Iter() iter.Iterator[Pair] {
	return iter.Map[[]string, Pair](iter.PairedSlice(h.headers), func(el []string) Pair {
		return Pair{Key: el[0], Value: el[1]}
	})
}
